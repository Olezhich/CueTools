...
def track_gen(filename : str, songname : str, tracks_count : int=1, tab: int =TAB) -> str:
...
def obj_sample_one_file_one_track() -> AlbumData:
    ...
    for i in range(1,8):
        album.add_track(TrackData(track=i, title=f'Song {i}', file=Path(f'0{i} - Song 0{i}.flac')))
    return album
...
def obj_sample_one_file_many_tracks() -> AlbumData:
    album = AlbumData(performer='The Performer', title='The Title Of Album', rem=RemData(genre='Rock', date=1969))
    for i in range(1,8):
        album.add_track(TrackData(track=i, title='song_name', file=Path('The Title Of Album.flac')))
    return album

@pytest.fixture()
def cue_sample_rem():
    album = album_rem_gen(genre='Hard Rock', date='1969', replaygain_album_gain='-4.10 db', replaygain_album_peak='0.987654')
    return '\n'.join(album)

@pytest.fixture()
def obj_sample_rem() -> AlbumData:
    album = AlbumData(rem=RemData(genre='Hard Rock', date=1969, replaygain_album_gain='-4.10 db', replaygain_album_peak='0.987654'))
    return album
